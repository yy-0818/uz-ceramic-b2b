...
const groups = computed(() => {
  ...
  return list
    .map((g) => {
      const convs = g.conversations.filter(
        (c) =>
...
<template>
  <div class="space-y-3">
    ...
    <!-- 列表加载错误: 把 supabase-js 400 详细信息暴露给用户 (后台排查) -->
    <div
      v-if="chat.listError.value"
      class="rounded-md border border-destructive/40 bg-destructive/5 px-3 py-2 text-xs text-destructive"
    >
      <div class="font-semibold mb-0.5">{{ t('chat.listLoadError') }}</div>
      <pre class="whitespace-pre-wrap break-all font-mono text-[11px] opacity-90">{{ chat.listError.value }}</pre>
    </div>
    ...
    <div class="rounded-2xl border bg-card px-3 py-2 flex items-center gap-2 flex-wrap">
      ...
      <div class="flex items-center gap-1">
        ...
        <Button size="sm" :variant="selectMode ? 'default' : 'outline'" @click="toggleSelectMode">
        ...
        >
          <div class="m-auto w-full max-w-xl mx-4 bg-card rounded-2xl border shadow-2xl flex flex-col max-h-[80dvh]">
            ...
            <div class="overflow-y-auto flex-1">
              <div
                v-if="!searchLoading && searchMsgKw && searchHits.length === 0"
    ...
    >
      ...
      <div class="flex items-center gap-1">
        <select v-model="batchAssignTo" class="text-xs rounded-md border border-input bg-background px-2 py-1.5">
    ...
    >
      ...
      >
        ...
        <template v-else>
          <div class="sticky top-0 z-10 bg-background/95 backdrop-blur px-3 py-2 border-b flex items-center gap-2">
            <Button size="icon" variant="ghost" class="md:hidden" @click="expandedGroupId = null">
            ...
            >
              <div class="flex items-start gap-2">
                ...
                <div class="min-w-0 flex-1">
                  ...
                  <p class="text-[11px] text-muted-foreground truncate">
                    {{ c.last_message ? c.last_message.body : t('chat.emptyMessages') }}
                  </p>
                ...
                />
                <Badge v-else-if="(c.unread_count ?? 0) > 0" variant="default" class="text-[10px] tabular-nums">
                  {{ c.unread_count }}
                </Badge>
              </div>
            </li>
      ...
      <section class="h-full">
        ...
        <div v-else class="h-full flex flex-col">
          ...
          <div class="px-3 py-2 border-b flex items-center gap-1 bg-muted/30">
            <Button size="icon" variant="ghost" class="md:hidden" @click="onBack">
              <ArrowLeft class="h-4 w-4" />
            </Button>
            <div class="flex-1 min-w-0">
            ...
            <Button v-else size="sm" variant="outline" :title="t('chat.reopenConversation')" @click="onReopen">
